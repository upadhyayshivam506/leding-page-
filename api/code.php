...
function process_region_assignments(string $batchId, array $assignments): array
{
    ...
    foreach ($assignments as $region => $selectedIds) {
        $normalizedRegion = normalize_region_name((string) $region);
        ...
        if (in_array('NONE', $selectedIds, true)) {
            $result['regions_skipped']++;
            $result['details'][] = [
                'region' => $normalizedRegion,
                'status' => 'skipped',
                'response' => 'Region skipped because None was selected.',
            ];
            continue;
        }
